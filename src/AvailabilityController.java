...
public class AvailabilityController implements Initializable {
	...
	public String username = "kupcha"; //in the future this will be set when scene is launched
	...
	public void initialize(URL location, ResourceBundle resources) {
		if (employeeType == 0) {
			...
			employeeGreeting.setText(username + ", you are a part-time employee and will be allocated less than 40 hours per week.");
		}
	}
	...
	public void handleSubmitButton() {
	    try
	    {
	      ...
	      String query;
	      if (openAvail == 0) {
	    	  query = "UPDATE `CompanyScheduler`.`availability` SET `monday` = '" + mondayAvailability + "', `tuesday` = '" + tuesdayAvailability + "', `wednesday` = '" + wednesdayAvailability + "', `thursday` = '" + thursdayAvailability + "', `friday` = '" + fridayAvailability + "' WHERE (`username` = '" + username + "');";
	      }else {
	    	  query = "UPDATE `CompanyScheduler`.`availability` SET `monday` = '0', `tuesday` = '0', `wednesday` = '0', `thursday` = '0', `friday` = '0' WHERE (`username` = '" + username + "');";
	      }
